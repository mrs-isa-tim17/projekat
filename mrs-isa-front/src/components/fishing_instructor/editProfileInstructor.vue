...
import FishingInstructorService from "@/servieces/FishingInstructorService";
import InstructorHeader from "@/components/insrtuctorHeader";
export default {
  name: "edit-profile-owner",
  components:{
    InstructorHeader, changePasswordModal,
  ...
  created:
      function () {
        ...
        FishingInstructorService.getOwner(this.coID)
            .then((response) => {
  ...
  methods: {
    ...
    updateProfile(){
      FishingInstructorService.updateOwner(this.coID,this.owner).then("Success");
