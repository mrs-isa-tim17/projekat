...
<div class="container p-3">
  ...
  <Multiselect style="width:300px;margin-top:7px;color:#31708E;"
               v-model="value"
               :options="cottagesName" placeholder="Odaberite vikendicu "
  />
...
export default {
  ...
  components:{cottageOwnerHeader,  Multiselect,Datepicker},
  created() {
  ...
  methods:{
    ...
    createQuickRes(){
      var cottage = "";
      for(let i=0;i<this.allCottages.length;i++){
        if(this.allCottages[i].name === this.value){
          cottage = this.allCottages[i];
          console.log(cottage);
        }
      }
      this.saleAppointmentDTO.longitude = cottage.longitude;
      this.saleAppointmentDTO.latitude = cottage.latitude;
      this.saleAppointmentDTO.price = this.price;
      this.saleAppointmentDTO.peopleQuantity = cottage.bedQuantity;
      this.saleAppointmentDTO.duration = this.duration;
      this.saleAppointmentDTO.offerId = cottage.id;
