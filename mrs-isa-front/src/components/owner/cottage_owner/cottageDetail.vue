...
<cottageOwnerHeader></cottageOwnerHeader>
  <div class="container" >
    ...
    <!--    <star-rating :rating="4" :starWidth="20" :star-style="config "></star-rating><br>    -->
       <experience-reviews-modal :header="experienceReviewsHeader" :index="experienceReviewsIndex" :cottage="cottageId"></experience-reviews-modal><br><br>
      ...
      <div class="row pt-2 mt-4">
        <div class="col-5" style="text-align: left">
         <p style="font-size:20px;display: inline-block;">{{cottage.description}}</p>
          ...
          <div class="col-1">
         <edit-offer-description :offerId="cottageId" :header="editDescHeader" :index="editDesc" :description="cottage.description" @edit-description="editDescription"></edit-offer-description>
          </div>
        ...
        <div style="text-align: left;font-size: 20px;text-decoration: underline;"><b>Dodatne usluge</b>
          <edit-offer-services-modal :offerId="cottageId" :key="servicesKey" :servicesList="services" :header="editServHeader" :index="editServ" @edit-services="editServices"></edit-offer-services-modal>
          <br>
        ...
        <div style="text-align: left;font-size: 20px;"><b>Kućni red</b>
          <edit-behavior-rules-modal :offerId="cottageId" :key="behaviorKey" :behaviorList="behavior" :header="behaviorHeader" :index="editBeh" @edit-rules="editRules"></edit-behavior-rules-modal>
          <br>
      ...
      <div class="col-md-2 " style="text-align: left;">
        ...
        <div style="font-size: 36px;"><i class="fa fa-users"></i> {{cottage.bedQuantity}}
          <edit-number-modal :offerId="cottageId" :header="editPeopleHeader" :index="editPeople" :number="cottage.bedQuantity" @edit-number="editCapacity"></edit-number-modal>
        </div>
        <div style="font-size: 36px;"> {{cottage.roomQuantity}} sobe
        <edit-number-modal :offerId="cottageId" :header="editRoomHeader" :index="editRoom" :number="cottage.roomQuantity" @edit-number="editRooms" ></edit-number-modal>
        </div>


        <div style="font-size: 28px;"> {{cottage.price}} din/noć
        <edit-price-modal :offerId="cottageId" :header="editPriceHeader" :index="editPriceIndex" :price="cottage.price" @edit-price="editPrice" ></edit-price-modal>
        </div>
        ...
        <div class="col-8" style="font-weight: bold;color:red;">Uslovi otkazivanja rezervacije</div>
            <div class="col-4">
            <edit-cancel-conditions-modal :offerId="cottageId" :key="ccKey" :header="editCCHeader" :index="editCCIndex" :p1="p1" :p2="p2" :p3="p3" :p4="p4" @edit-cancel-condition="editCancelConditions" ></edit-cancel-conditions-modal></div>
         <br>
...
export default {
  ...
  created() {
    ...
    ReservationService.getFutureReservations(id).then((response) => {
          console.log(response.data);
          if (response.data.length > 0) {
            var message = "Za vikendicu " + this.cottage.name + " postoje nerealizovane rezervacije, tako da se ne može menjati!"
            swal.fire({
              title: message,
              background: 'white',
              color: '#687864',
  ...
  methods: {
    ...
    editOfferName(name) {
      ...
      if (name == "") {
        ...
        ReservationService.getFutureReservations(this.cottage.id).then((response) => {
          if (response.data.length > 0) {
            var message = "Za vikendicu " + this.cottage.name + " postoje nerealizovane rezervacije, tako da se ne može menjati!"
            swal.fire({
              title: message,
        ...
        if (response.data.length > 0) {
          ...
          CottageService.updateCottage(this.cottage, this.cottage.id).then((response) => {
            ...
            if (this.cottage.roomQuantity == response.data.roomQuantity) {
              ...
              swal.fire({
                title: 'Uspešno izmenjen broj soba!',
                background: 'white',
                color: '#687864',
                confirmButtonColor: '#687864'
              });
