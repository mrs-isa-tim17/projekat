    ...
    <div  style="border:1px solid #31708E;background-color:#31708E;color:white; width: 1000px; display: inline-block;">
      <div class="row" >
        ...
        <div class="col-4">
          <br>
          <br>
          <label >Klijenti koji su trenutno na Vašim brodovima</label>
      ...
      <div class="row">
        ...
        <div class="col-3" >
          ...
          <Datepicker v-model="startDate" @change="enteredStartDate" style="width:200px;margin-left:55px;"></Datepicker>
        </div>
        <div class="col-1"></div>
        <div class="col-3" style="text-align: left;">
          <label ><b>Krajnji datum</b></label><br>
          <Datepicker v-model="endDate" @change="enteredEndDate" style="width:200px;"></Datepicker>
        </div>
        <div class="col-5">
...
export default {
  name: "newShipReservationForClient",
  components: {//CalendarForOneCottage,
   shipOwnerHeader,Multiselect,Datepicker},
  created() {
    console.log(this.index);
    this.modalId = "#" + this.index;
    this.coID = JSON.parse(localStorage.user).id;
    ClientServce.getCurrentShipClients(this.coID).then((response)=>
    {
      ...
      reservationServce.reserveEntity(resObj)
          .then((response) =>{
            ...
            if (res.successfull){
              swal.fire({
                title: "Uspešno",
                text: "Uspešno je sačuvana rezervacija, email o potvrdi rezervacije je poslat na Vašu email adresu.",
                background:'white',
                color:'black',
